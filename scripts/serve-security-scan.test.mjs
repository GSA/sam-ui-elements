...
before(async () => {
  // The server serves `test-app/dist/browser` relative to its cwd; build a
  // temp repo-shaped dir and run the server there.
  docroot = mkdtempSync(join(tmpdir(), "serve-scan-"));
  const dist = join(docroot, "test-app", "dist", "browser");
