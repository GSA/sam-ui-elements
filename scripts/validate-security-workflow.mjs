 ...
 * round-trips through Date -- rejecting values like 9999-99-99 or
 * 2027-02-30 that pass a naive \d{4}-\d{2}-\d{2} regex but are not real
 * dates.
 */
...
/**
 * Policy validator for the security CI configuration. Runs in the main CI
 * job (`npm run validate:security-workflow`) so that any drift in the
 * security workflow, the ZAP severity gate, the reviewed exception
 * baseline, or the documentation is caught in-repo rather than silently
 * weakening the gate.
 *
 * The checks intentionally assert on the *shape* of the configuration (the
 * gate exists, is least-privilege, does not disable the severity check,
 * keeps Dependabot, documents the admin-owned required checks) rather than
 * running the containerized CodeQL / ZAP scans, which only run in GitHub
 * Actions.
 */
...
if (!existsSync(workflowPath)) {
  ...
  const checks = [
    [
      "runs for pull requests and pushes to master",
    ...
    [
      "does not grant security-events write in this workflow",
      !/^\s*security-events:\s*write\s*$/m.test(workflow),
    ],
    [
      "defers SAST to the repository CodeQL default setup",
      !/github\/codeql-action\//.test(workflow),
    ],
    [
      "builds test-app before DAST",
      /dast:\s*\n[\s\S]*npm --prefix test-app run build -- --configuration production/,
    ],
    [
      "serves the runtime with production security headers",
      /node scripts\/serve-security-scan\.mjs/,
    ],
    [
    ...
    [
      "does not hand the exception file to ZAP as a rules_file_name config",
      !/rules_file_name:/.test(workflow),
    ],
  ];

  for (const [description, condition] of checks) {
    const passed =
      condition instanceof RegExp ? condition.test(workflow) : condition;
...
if (
  ...
  !/npm run validate:security-workflow/.test(
...
if (!existsSync(rulesPath)) {
  ...
  for (const row of exceptionRows) {
    const [ruleId, action, scope, issue, owner, expiry, rationale, ...extra] =
      row.split("\t");
    if (
      !/^\d+$/.test(ruleId) ||
      action !== "IGNORE" ||
      !scope?.trim() ||
      !/^https:\/\/github\.com\/GSA\/sam-ui-elements\/issues\/\d+$/.test(
        issue
      ) ||
      !owner?.trim() ||
...
if (!existsSync(serverPath)) {
  ...
  if (
    !/Content-Security-Policy/.test(server) ||
    !/X-Frame-Options/.test(server)
  ) {
...
if (!existsSync(documentationPath)) {
  ...
  const documentationChecks = [
    ...
    [
      "documents CodeQL new-alert merge protection",
      /code scanning.*new.*alert/is,
    ],
    ["names the DAST required status check", /DAST \(medium\/high gate\)/],
    ["documents exception rationale and expiry", /rationale[\s\S]*expir/i],
    [
      "states that Dependabot remains complementary",
      /Dependabot[\s\S]*complement/i,
    ],
  ];
