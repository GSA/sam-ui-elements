 ...
 * real CLI surface (exit codes, --bump, malformed floors) rather than
 * internals.
 *
...
function writeSummary(dir, pcts) {
  const total = Object.fromEntries(
    Object.entries(pcts).map(([k, v]) => [k, { pct: v }])
  );
 ...
 * so we back it up rather than parameterise the path.
 */
...
test("a malformed floor (string/null) fails the gate without emitting NaN", () => {
  withFloors(
    { statements: "53", branches: null, functions: 50, lines: 52 },
    () => {
      withTempDir((dir) => {
        const summary = writeSummary(dir, {
          statements: 85,
...
test("exits non-zero when the coverage summary is missing", () => {
  const { status, stderr } = run([
    join(tmpdir(), "does-not-exist-covgate.json"),
  ]);
  assert.equal(status, 1);
