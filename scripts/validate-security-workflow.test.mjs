...
test("fails when the CI workflow does not run the validator", () => {
  const result = withMirror((dir) => {
    ...
    const ci = readFileSync(path, "utf8").replace(
      /npm run validate:security-workflow/g,
      "noop"
    );
...
test("rejects a malformed ZAP exception row", () => {
  const result = withMirror((dir) => {
    writeFileSync(
      join(dir, ".zap/rules.tsv"),
      "not-a-number\tIGNORE\t*\tno-url\t\t\t\n"
    );
    return run(dir);
  });
  assert.equal(result.status, 1);
...
test("rejects an expired ZAP exception row", () => {
  const result = withMirror((dir) => {
    const row =
      "10038\tIGNORE\t*\thttps://github.com/GSA/sam-ui-elements/issues/1\towner\t2000-01-01\told";
...
test("rejects a row with a syntactically valid but impossible calendar date", () => {
  const result = withMirror((dir) => {
    const row =
      "10038\tIGNORE\t*\thttps://github.com/GSA/sam-ui-elements/issues/1\towner\t2027-02-30\treason";
    writeFileSync(join(dir, ".zap/rules.tsv"), `${row}\n`);
    return run(dir);
  });
  assert.equal(result.status, 1);
...
test("rejects a row with an out-of-range date (9999-99-99)", () => {
  const result = withMirror((dir) => {
    const row =
      "10038\tIGNORE\t*\thttps://github.com/GSA/sam-ui-elements/issues/1\towner\t9999-99-99\treason";
    writeFileSync(join(dir, ".zap/rules.tsv"), `${row}\n`);
    return run(dir);
  });
  assert.equal(result.status, 1);
...
test("rejects a row missing the scope column", () => {
  const result = withMirror((dir) => {
    // 6 columns (no scope) must be rejected under the 7-column layout.
    const row =
      "10038\tIGNORE\thttps://github.com/GSA/sam-ui-elements/issues/1\towner\t2999-01-01\treason";
    writeFileSync(join(dir, ".zap/rules.tsv"), `${row}\n`);
    return run(dir);
  });
  assert.equal(result.status, 1);
...
test("fails when Dependabot config is removed", () => {
  const result = withMirror((dir) => {
    rmSync(join(dir, ".github/dependabot.yml"));
    return run(dir);
  });
  assert.equal(result.status, 1);
...
test("fails when the exception file is handed to ZAP as a rules_file_name config", () => {
  const result = withMirror((dir) => {
    const path = join(dir, ".github/workflows/security.yml");
    const workflow = readFileSync(path, "utf8").replace(
  ...
  assert.equal(result.status, 1);
  assert.match(result.stderr, /does not hand the exception file to ZAP/);
});
