#!/usr/bin/env node
/**
 * OWASP ZAP severity gate for the test-app demo runtime.
 *
 * The DAST job (`.github/workflows/security.yml`) runs the ZAP baseline
 * scanner with `fail_action: false`, so ZAP's own rule actions (WARN/FAIL)
 * never decide the build outcome. This script does: it parses ZAP's JSON
 * report and fails the build for any alert whose JSON `riskcode` is medium
 * (`2`) or high (`3`), unless a reviewed exception in `.zap/rules.tsv`
 * matches it.
 *
 * Fail-closed posture:
 ...
 *
 * Exception scope:
 *   Exceptions are matched on plugin id AND an instance scope (a URL
 *   substring), so a baseline row suppresses only the reviewed instance(s)
 *   of a finding, not every current and future instance of that ZAP rule
 *   across all URLs. A literal `*` in the scope column means rule-wide and
...
// Fail closed on a report shape we do not recognise: ZAP always emits a
// `site` array (even if empty). A missing/non-array `site`, or a `site`
// entry whose `alerts` is not an array, means the report is truncated or
// schema-changed and must not be read as "no findings".
if (
  report === null ||
...
// A non-numeric riskcode is treated as blocking so a corrupted severity
// field cannot silently downgrade a finding below the gate.
...
if (suspicious.length > 0) {
  ...
  for (const alert of suspicious) {
    console.error(
      `- [${alert.pluginid ?? "?"}] ${alert.alert ?? "unknown"}: riskcode=${alert.riskcode}`
    );
  }
  process.exit(1);
}
...
if (blockingAlerts.length > 0) {
  ...
  for (const alert of blockingAlerts) {
    console.error(
      `- [${alert.pluginid}] ${alert.alert}: ${alert.riskdesc} (${alert.url ?? "no url"})`
    );
  }
  process.exit(1);
}
