...
@Component({
  ...
  template: `
    <form [formGroup]="group" (ngSubmit)="runReportEvent.next($event)">
      <ng-content></ng-content>
      <sam-button-next action="submit" [isDisabled]="disableAction">
        {{ runBtnText }}
      </sam-button-next>
      <sam-button-next
        action="secondary"
...
export class SamFiltersWrapperComponent implements OnInit, OnDestroy {
  ...
  private _initializeHandlers() {
    ...
    this._resetReportSubscription = this.resetReportEvent.subscribe(
  ...
  private _clearModel() {
    const cleared = {};
    Object.keys(this.group.value).forEach((key) => (cleared[key] = null));
    if (this._service) {
      this._service.model.properties["filters"].setValue(cleared);
    }
  }
  ...
  private _initializeService() {
    if (this._service) {
      ...
      this._filtersSubscription = this._service.model.properties[
        "filters"
