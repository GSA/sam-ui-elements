...
@Component({
  ...
  providers: [AccessorToken(SamTelephone), ValidatorToken(SamTelephone)],
  standalone: false,
})
export class SamTelephone extends SamFormControl implements OnChanges {
  ...
  public ngOnChanges(c: SimpleChanges): void {
    if (c.countryCode) {
      this.setValidatorByCountry(c.countryCode.currentValue);
  ...
  public handleFocus(event: any): void {
    ...
    if (target) {
      this.inputValue = this.templateToNumber(target.value);
    }
  }
  ...
  public handleBlur(event: any): void {
    ...
    if (target) {
      this.inputValue = this.numberToTemplate(target.value);
    }
  }
  ...
  private numberToTemplate(numberStr: string): string {
    ...
    const blanks = this.template ? this.template.split("") : [];
