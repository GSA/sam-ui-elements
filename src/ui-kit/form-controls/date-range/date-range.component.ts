...
export class SamDateRangeComponent
  ...
  public static dateRangeRequired(instance: SamDateRangeComponent) {
    ...
    return (c: AbstractControl) => {
      if (fromRequired && toRequired) {
        const valid = !(
          (c.value && c.value.startDate === "Invalid date") ||
          (c.value && c.value.endDate === "Invalid date")
        );
  ...
  private static validateStart(c) {
    ...
    if (
      c.value &&
      !(c.value.startDate && c.value.startDate === "Invalid date")
    ) {
      const startDateM = moment(c.value.startDate);
  ...
  parseValueString() {
    const format =
      this.type !== "date-time" ? this.INPUT_FORMAT : this.DT_INPUT_FORMAT;
  ...
  dateChange() {
    ...
    if (!this.isEmptyField(this.startModel)) {
      startDateString = this.getDate(this.startModel).format(
        this.OUTPUT_FORMAT
      );
    }
