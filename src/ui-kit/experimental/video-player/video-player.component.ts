...
export class SamVideoPlayerComponent {
  ...
  public tracks: QueryList<ElementRef>;
  @ContentChildren("videoSrc", { descendants: true })
  public sources: QueryList<ElementRef>;
  ...
  ngAfterContentInit() {
    this.validateElement(
      this.videos,
      "SamVideoComponent must be provide a <video> element to function or provide template variable #videoPly"
    );
    this.validateElement(
      this.sources,
      "SamVideoComponent must be provide a <source> element to function or provide template variable #videoSrc"
    );
    this.validateElement(
      this.tracks,
      "SamVideoComponent must be provide a <track> element with captions for 508 compliance or provide template variable #videoTrack"
    );
  }
