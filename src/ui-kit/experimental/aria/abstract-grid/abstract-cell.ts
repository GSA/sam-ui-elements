...
export class AbstractCell {
  ...
  private _dispatcher: EventDispatcher = new EventDispatcher(this._cellEvents);
  ...
  private _findTabbableChildren() {
    this._tabbableChildren = this.node.querySelectorAll(tabbable);
  }
  ...
  private _addTabbableChildren() {
    ...
    for (let i = 0; i < length; i++) {
      (<HTMLElement>this._tabbableChildren[i]).setAttribute("tabindex", "0");
    }
  }
  ...
  private _removeTabbableChildren() {
    ...
    for (let i = 0; i < length; i++) {
      (<HTMLElement>this._tabbableChildren[i]).setAttribute("tabindex", "-1");
    }
  }
