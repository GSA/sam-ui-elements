...
public ngOnInit() {
  console.log('ngOnInit - SamDollarComponent');
  this.setupComponent();
}

protected setupComponent(){
  console.log('setupComponent - SamDollarComponent');
  if (!this.id) {
...
public onLoseFocus() {
  ...
  this.value = this.numberWithCommas(value);
  this.onBlur.emit(true);
  this.blur.emit(true);
  this.cdr.detectChanges();
}
...
public roundForCurrency(number) {
  if (isNaN(number)) {
    return "";
  }
