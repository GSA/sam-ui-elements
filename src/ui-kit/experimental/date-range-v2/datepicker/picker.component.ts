  ...
  implements OnInit, OnChanges, ControlValueAccessor
{
  ...
  @HostListener("document:click", ["$event"]) documentClickHandler(
    event: MouseEvent
  ) {
  ...
  handleGlobalClick(event: MouseEvent): void {
    if (this.showCalendar && this.calendarpopup) {
      const withinElement = this.calendarpopup.nativeElement.contains(
        event.target
      );
      if (
        !withinElement &&
  ...
  isChosenDay(day: Date): boolean {
    if (day) {
      return this.date
        ? day.toDateString() === this.date.toDateString()
        : false;
    } else {
      return false;
    }
  ...
  isHoveredDay(day: Date): boolean {
    return this.hoveredDay
      ? this.hoveredDay === day && !this.isChosenDay(day)
      : false;
  }
  ...
  inRangeValidator(control: FormControl): ValidationResult {
    ...
    if (this.currentMonthNumber) {
      ...
      if (
        this.rangeStart &&
        tentativeDate.getTime() < this.rangeStart.getTime()
      ) {
  ...
  yearValidator(control: FormControl): ValidationResult {
    const value = control.value;
    const valid =
      !isNaN(value) && value >= 1970 && Math.floor(value) === +value;
    if (valid) {
      return null;
    }
